...
export default function StudentLoginPage() {
  ...
  async function signInGoogle() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${siteUrl()}/auth/callback?next=/student` },
    });
    if (error) {
      ...
      <section className="relative hidden overflow-hidden bg-brand-ink lg:flex lg:flex-col lg:justify-between lg:p-12">
        ...
        <div className="relative">
          <h2 className="max-w-md text-3xl font-semibold leading-tight text-white">
            Your profile, in front of hiring companies.
          </h2>
          <p className="mt-4 max-w-md text-base leading-relaxed text-slate-300">
            Sign in to see how companies see you and keep your resume up to
            date.
          </p>
        </div>
      ...
      <section className="flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Use the email you applied with. We&apos;ll send you a sign-in link —
            no password needed.
          </p>
          ...
          <p className="mt-6 text-xs leading-relaxed text-slate-400">
            Invited by your school or bootcamp? Use the link in your invite
            email to get started, then come back here to sign in.
          </p>
        </div>
      </section>
    </main>
